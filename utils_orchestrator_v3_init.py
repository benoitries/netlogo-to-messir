...
from utils_config_constants import AGENT_CONFIGS
...
def initialize_v3_orchestrator_components(orchestrator_instance, model_name: str):
    ...
    orchestrator_instance.persona_set = "persona-v3-limited-agents"
    orchestrator_instance.timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M")
    orchestrator_instance.selected_persona_set = "persona-v3-limited-agents"
    orchestrator_instance.agent_configs = AGENT_CONFIGS.copy()
    orchestrator_instance.processed_results = {}
    
    # Initialize agents
    orchestrator_instance.lucim_operation_synthesizer_agent = LucimOperationModelGeneratorAgent(model_name, orchestrator_instance.timestamp)
    ...
    orchestrator_instance.token_usage = {
        "lucim_operation_generator": {"used": 0},
    ...
    orchestrator_instance.detailed_timing = {
        "lucim_operation_generator": {"start": 0, "end": 0, "duration": 0},
    ...
    tool_added = configure_agent_with_adk_tools(
        ...
        "lucim_operation_generator"
    )
    
    if hasattr(orchestrator_instance, 'logger') and orchestrator_instance.logger:
        if tool_added:
            orchestrator_instance.logger.info("[ADK] GoogleSearchTool successfully configured for lucim_operation_generator")
        else:
            orchestrator_instance.logger.info("[ADK] No ADK tools configured for lucim_operation_generator (tool not available or not needed)")
